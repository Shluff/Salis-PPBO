...
public class Gudang {
    ...
    public void buatBarang(Toko.TipeBarang barang, int jumlah) {
        ...
        if (bahanPembuatanMencukupi(kayuDibutuhkan, bautDibutuhkan, catDibutuhkan)) {
            ...
            System.out.println("Gagal membuat " + jumlah + " " + barang + ", stok bahan tidak cukup");
        }
    }
    ...
    public void masukanBarangKeGudang(Toko.TipeBarang barang, int jumlah) {
        if (slotTersedia()) {
            ...
            System.out.println("Gagal memasukkan " + jumlah + " " + barang + " ke dalam gudang, kapasitas gudang penuh");
        }
    }
    ...
    public void keluarkanBarangDariGudang(Toko.TipeBarang barang, int jumlah) {
        if (isiGudang.get(barang) >= jumlah) {
            isiGudang.put(barang, isiGudang.get(barang) - jumlah);
            System.out.println("Berhasil mengeluarkan " + jumlah + " " + barang + " dari gudang");
        } else {
            System.out.println("Gagal mengeluarkan " + jumlah + " " + barang + " dari gudang, stok tidak mencukupi");
        }
    }

    private boolean bahanPembuatanMencukupi(int kayuDibutuhkan, int bautDibutuhkan, int catDibutuhkan) {
        return stokBahan.get(Bahan.KAYU) >= kayuDibutuhkan && stokBahan.get(Bahan.BAUT) >= bautDibutuhkan && stokBahan.get(Bahan.CAT) >= catDibutuhkan;
    }
}
